...
두 서브에이전트를 **병렬**로 실행. 각 에이전트는 worktree에서 작업.
파일 소유권 겹침 없음. 각각 브랜치 생성 후 PR.
...
2. 의존성 추가:
   - Runtime: `nest-commander`, `pdf-lib`, `@pdf-lib/fontkit`, `pdf-parse`, `class-validator`, `class-transformer`, `@nestjs/config`
...
3. 디렉토리 구조 생성 (CLAUDE.md Directory Structure 참조):
   - `src/pdf/` — interfaces (IPdfExtractor + PDF_EXTRACTOR token, IPdfGenerator + PDF_GENERATOR token), services (stub), dto, controller, module
...
6. 모듈 와이어링:
   ...
   - PdfModule → PDF_EXTRACTOR, PDF_GENERATOR 토큰 provide/export
   - TranslationModule → Factory + 서비스들 provide/export
7. main.ts: `ValidationPipe({ transform: true })`
...
11. `pnpm run lint` + `pnpm test` 통과 확인
...
8. `README.md` — 프로젝트 소개, CLI 사용법, HTTP API 사용법 (curl 예제), Docker 사용법, Phase 로드맵
9. `docs/architecture.md` — Mermaid 다이어그램 (모듈 의존성 그래프, 요청 흐름 시퀀스, 어댑터 패턴 설명), 환경변수 레퍼런스 테이블
...
## Done Criteria
- 양쪽 PR 생성 완료
- backend-builder: `pnpm run lint` + `pnpm test` 통과
- 파일 소유권 겹침 없음
