...
각 에이전트는 자동 교정 루프(build → lint → test)를 통과한 후 commit → push → PR 생성.
...
5. stub 메서드: `throw new Error('Not implemented: Phase 1')`
6. 모듈 와이어링:
...
11. 자동 교정 루프: `pnpm build` + `pnpm lint` + `pnpm test` 통과 확인
12. ## 자동 교정 + 리뷰 루프
