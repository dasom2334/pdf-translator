...
## 질의 사항
의도가 불분명한 항목이 있을 경우 작성. 오케스트레이터가 워커에게 전달하고, 답변을 받으면 A1에 채워 넣는다.
- Q1: ...
- A1: (오케스트레이터가 워커 확인 후 기입)
...
**ROUND > 1이고 이전 라운드에 질의 사항이 있었던 경우:** 코멘트에 아래 섹션을 반드시 포함한다.
```
## 질의 응답

- Q1: {이전 라운드의 질문}
- A1: {오케스트레이터로부터 전달받은 답변. 미전달 시 "미확인 — 수정 요청으로 전환" 표기}
```
...
오케스트레이터가 이 질의를 워커에게 전달하여 의도를 확인한 후, A{N}에 답변을 채워 재검수한다.
- Q1: {파일}:{줄} — {질문}
- A1: {오케스트레이터가 워커 확인 후 기입 — 답변이 없으면 "미확인"으로 표기}
