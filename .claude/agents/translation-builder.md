...
6. **code-reviewer 검수:**
   ...
   CONTEXT: {ROUND > 1이고 사용자 피드백으로 인한 재작업인 경우 — 누가 어떤 문제를 제기했는지, 왜 문제인지, 어떤 결론이 났는지 작성. 단순 REQUEST_CHANGES 재작업이면 생략}
   ```
   ...
   - REQUEST_CHANGES:
     ...
     2. PR에 ✅ 수정 완료 코멘트 게시:
        ```
        ## ✅ 수정 완료
        **논의 주체:** code-reviewer ↔ translation-builder
        ### 수정 내용
        {항목별 — 무엇을, 어떻게, 왜 그렇게 수정했는지}
        ### 적용 여부
        {✅ 적용 완료 / ⏭️ 향후 과제로 이관 — 이관 시 이유 명시}
        ```
     3. 1번부터 재시작 (ROUND +1, 최대 3회)
   - APPROVE → 7번으로 이동
