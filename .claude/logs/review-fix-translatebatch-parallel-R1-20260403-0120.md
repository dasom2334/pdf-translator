...
- 변경된 코드 경로(translateBatch)는 translate()를 위임하므로 예외 처리 변화 없음
- BadRequestException / TranslationException 분류 그대로 유지: 준수

### 파일 소유권
- translation-builder 소유 파일만 수정 (src/translation/**): 준수
...
### 테스트 커버리지
- 기존 happy path 케이스 유지 (설명만 변경)
- 신규: `vi.spyOn(service, 'translate')`로 호출 횟수 검증 — 병렬성 간접 검증
- 빈 배열 케이스 유지
- 에러 케이스는 translate() 단위 테스트에서 이미 커버됨

### 성능 / 부작용
- MyMemory: 일일 한도(5000자)가 있으므로 병렬 처리 시 burst 가능성 있으나,
  translateBatch 호출 빈도가 낮고 청크 단위 제어는 translate() 내부에서 유지되므로 허용 범위 내
- Gemini: rate limit 대응 로직(지수 백오프)이 translate() 내 translateChunkWithRetry에 구현되어 있어
  병렬 전환 후에도 재시도 동작 유지됨
